...
public final class Matrices {
    ...
    public static final Mat4 perspective(final float fovy, final float aspect, final float zNear, final float zFar) {
        final float radians = (float) FastMath.toRadians((fovy / 2.0f));
        final float sine = (float) Math.sin(radians);
        final float deltaZ = zFar - zNear;
        final float cotan = (float) Math.cos(radians) / sine;
        return new Mat4(
                cotan / aspect, 0f, 0f, 0f,
                0f, cotan, 0f, 0f,
                0f, 0f, -(zFar + zNear) / deltaZ, -1f,
                0f, 0f, -(2f * zFar * zNear) / deltaZ, 0f
        );
    }
